...
const reviewTemplates = {
  ebooks: {
    positive: [
      ...
      'دليل شامل ومفيد للمبتدئين',
      'كتاب يوفر الكثير من الوقت والجهد',
      'معلومات حديثة ومواكبة للعصر',
      'أسلوب شرح بسيط ومفهوم',
      'كتاب عملي يمكن تطبيقه فوراً'
    ],
    neutral: [
      'كتاب جيد بشكل عام',
      ...
      'جيد للمبتدئين',
      'كتاب أساسي في الموضوع',
      'محتوى مناسب للمستوى المتوسط'
    ],
    negative: [
...
// عدد التقييمات المطلوب لكل منتج (الأعداد المحدثة)
const reviewCounts = {
  1: 65,  // الدليل التمهيدي للربح من المنتجات الرقمية
  2: 75,  // الربح من المنتجات الرقمية
  3: 75,  // 15 فكرة مشروع رقمي مربح
  4: 10,  // باقة المونتاج الاحترافية
  5: 198, // حزمة أيقونات متحركة (بدون تغيير)
  6: 45   // باقة التصميم وصناعة المحتوى (بدون تغيير)
};
...
products.forEach(product => {
  ...
  const count = reviewCounts[productId] || 50; // افتراضي 50 تقييم
  ...
  console.log(`✅ تم توليد ${count} تقييم للمنتج: ${product.product_name}`);
});
...
console.log('📈 توزيع التقييمات حسب المنتج:');
Object.keys(allReviews).forEach(productId => {
  const product = products.find(p => p.product_id == productId);
  console.log(`   - ${product.product_name}: ${allReviews[productId].length} تقييم`);
});
