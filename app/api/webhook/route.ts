...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`  📥 Download URL: ${matchedProduct.image}`);
    console.log(`  📄 Filename: ${matchedProduct.name}`);
    console.log(`  💰 Price: ${matchedProduct.price} ${"SAR"}`);
    ...
    const paymentRecord = {
      ...
      download_url: matchedProduct.image,
      filename: matchedProduct.name,
      file_size_mb: 1.0,
    ...
    console.log(`💳 Payment ID: ${paymentId}`);
    console.log(`📦 Order Number: ${orderNumber}`);
    console.log(`💰 Amount: ${amount} ${currencyCode || "SAR"}`);
